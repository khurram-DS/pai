...
#table 5
    ...
    #table7
          ...
          #table9
            ...
            # table43
            ta43=df['الأبواب'].astype(str).str.split('-',expand=True)
            ma43=pd.DataFrame({})
            ma43['الكود (ISIC 4)'] = df['ISIC4'].astype(str).str[:2]
            ta43=df['الأبواب'].astype(str).str.split('-',expand=True)
            cl=ta43[1].str.replace('\d+','')
            ma43['البيان']=cl.str.strip()
            ma43['الكود (ISIC 4)']=ma43['الكود (ISIC 4)'].replace(['38','43','73','82'],'32')
            ma43['البيان']=ma43['البيان'].replace(['أنشطة جمع النفايات ومعالجتها وتصريفها ، واسترجاع المواد','أنشطة التشييد المتخصصة','أبحاث الإعلان والسوق','الأنشطة الإدارية للمكاتب ، وأنشطة الدعم للمكاتب وغير ذلك من أنشطة الدعم للأعمال'],'الصناعة التحويلية الأخرى')
            ma43['كويتي']=df['الإدارة العليا والإدارة المساعدة-كويتي']
            ma43['خليجي']=df['الإدارة العليا والإدارة المساعدة-خليجي']
            ma43['عربي']=df['الإدارة العليا والإدارة المساعدة-عربي']
            ma43['أجنبي']=df['الإدارة العليا والإدارة المساعدة-أجنبي']
            col43=['كويتي','خليجي','عربي','أجنبي']
            ma43[col43]=ma43[col43].apply(pd.to_numeric, errors='coerce').fillna(0).astype(np.int64,errors='ignore')
            ma43['الإجمالي']=ma43['كويتي']+ma43['خليجي']+ma43['عربي']+ma43['أجنبي']
            tab43=pd.DataFrame({})
            if option == 'Sum':
                tab43=ma43.groupby(['الكود (ISIC 4)','البيان'])[['كويتي','خليجي','عربي','أجنبي','الإجمالي']].sum()
            if option == 'Mean':
                tab43=ma43.groupby(['الكود (ISIC 4)','البيان'])[['كويتي','خليجي','عربي','أجنبي','الإجمالي']].mean()
            tab43.reset_index(inplace=True)
            tab43.loc['إجمالي المنشآت الصناعية']=tab43.sum(numeric_only=True, axis=0)
            tab43['الكود (ISIC 4)']=tab43['الكود (ISIC 4)'].replace(np.nan,'')
            tab43['البيان']=tab43['البيان'].replace(np.nan,'إجمالي المنشآت الصناعية')
            tab43=tab43.reset_index(drop=True)
            col43=['كويتي','خليجي','عربي','أجنبي','الإجمالي']
            tab43[col43]=tab43[col43].astype(np.int64,errors='ignore')
            da43=tab43.style.apply(highlight_col, axis=None)

            # table44
            ta44=df['الأبواب'].astype(str).str.split('-',expand=True)
            ma44=pd.DataFrame({})
            ma44['الكود (ISIC 4)'] = df['ISIC4'].astype(str).str[:2]
            ta44=df['الأبواب'].astype(str).str.split('-',expand=True)
            cl=ta44[1].str.replace('\d+','')
            ma44['البيان']=cl.str.strip()
            ma44['الكود (ISIC 4)']=ma44['الكود (ISIC 4)'].replace(['38','43','73','82'],'32')
            ma44['البيان']=ma44['البيان'].replace(['أنشطة جمع النفايات ومعالجتها وتصريفها ، واسترجاع المواد','أنشطة التشييد المتخصصة','أبحاث الإعلان والسوق','الأنشطة الإدارية للمكاتب ، وأنشطة الدعم للمكاتب وغير ذلك من أنشطة الدعم للأعمال'],'الصناعة التحويلية الأخرى')
            ma44['كويتي']=df['إدارة فنية وعمالة انتاج-كويتي']
            ma44['خليجي']=df['إدارة فنية وعمالة انتاج-خليجي']
            ma44['عربي']=df['إدارة فنية وعمالة انتاج-عربي']
            ma44['أجنبي']=df['إدارة فنية وعمالة انتاج-أجنبي']
            col44=['كويتي','خليجي','عربي','أجنبي']
            ma44[col44]=ma44[col44].apply(pd.to_numeric, errors='coerce').fillna(0).astype(np.int64,errors='ignore')
            ma44['الإجمالي']=ma44['كويتي']+ma44['خليجي']+ma44['عربي']+ma44['أجنبي']
            tab44=pd.DataFrame({})
            if option == 'Sum':
                tab44=ma44.groupby(['الكود (ISIC 4)','البيان'])[['كويتي','خليجي','عربي','أجنبي','الإجمالي']].sum()
            if option == 'Mean':
                tab44=ma44.groupby(['الكود (ISIC 4)','البيان'])[['كويتي','خليجي','عربي','أجنبي','الإجمالي']].mean()
            tab44.reset_index(inplace=True)
            tab44.loc['إجمالي المنشآت الصناعية']=tab44.sum(numeric_only=True, axis=0)
            tab44['الكود (ISIC 4)']=tab44['الكود (ISIC 4)'].replace(np.nan,'')
            tab44['البيان']=tab44['البيان'].replace(np.nan,'إجمالي المنشآت الصناعية')
            tab44=tab44.reset_index(drop=True)
            col44=['كويتي','خليجي','عربي','أجنبي','الإجمالي']
            tab44[col44]=tab44[col44].astype(np.int64,errors='ignore')
            da44=tab44.style.apply(highlight_col, axis=None)

            # table45
            ta45=df['الأبواب'].astype(str).str.split('-',expand=True)
            ma45=pd.DataFrame({})
            ma45['الكود (ISIC 4)'] = df['ISIC4'].astype(str).str[:2]
            ta45=df['الأبواب'].astype(str).str.split('-',expand=True)
            cl=ta45[1].str.replace('\d+','')
            ma45['البيان']=cl.str.strip()
            ma45['الكود (ISIC 4)']=ma45['الكود (ISIC 4)'].replace(['38','43','73','82'],'32')
            ma45['البيان']=ma45['البيان'].replace(['أنشطة جمع النفايات ومعالجتها وتصريفها ، واسترجاع المواد','أنشطة التشييد المتخصصة','أبحاث الإعلان والسوق','الأنشطة الإدارية للمكاتب ، وأنشطة الدعم للمكاتب وغير ذلك من أنشطة الدعم للأعمال'],'الصناعة التحويلية الأخرى')
            ma45['كويتي']=df['عمال الخدمات والصيانة-كويتي']
            ma45['خليجي']=df['عمال الخدمات والصيانة-خليجي']
            ma45['عربي']=df['عمال الخدمات والصيانة-عربي']
            ma45['أجنبي']=df['عمال الخدمات والصيانة-أجنبي']
            col45=['كويتي','خليجي','عربي','أجنبي']
            ma45[col45]=ma45[col45].apply(pd.to_numeric, errors='coerce').fillna(0).astype(np.int64,errors='ignore')
            ma45['الإجمالي']=ma45['كويتي']+ma45['خليجي']+ma45['عربي']+ma45['أجنبي']
            tab45=pd.DataFrame({})
            if option == 'Sum':
                tab45=ma45.groupby(['الكود (ISIC 4)','البيان'])[['كويتي','خليجي','عربي','أجنبي','الإجمالي']].sum()
            if option == 'Mean':
                tab45=ma45.groupby(['الكود (ISIC 4)','البيان'])[['كويتي','خليجي','عربي','أجنبي','الإجمالي']].mean()
            tab45.reset_index(inplace=True)
            tab45.loc['إجمالي المنشآت الصناعية']=tab45.sum(numeric_only=True, axis=0)
            tab45['الكود (ISIC 4)']=tab45['الكود (ISIC 4)'].replace(np.nan,'')
            tab45['البيان']=tab45['البيان'].replace(np.nan,'إجمالي المنشآت الصناعية')
            tab45=tab45.reset_index(drop=True)
            col45=['كويتي','خليجي','عربي','أجنبي','الإجمالي']
            tab45[col45]=tab45[col45].astype(np.int64,errors='ignore')
            da45=tab45.style.apply(highlight_col, axis=None)

            # table46
            ta46=df['الأبواب'].astype(str).str.split('-',expand=True)
            ma46=pd.DataFrame({})
            ma46['الكود (ISIC 4)'] = df['ISIC4'].astype(str).str[:2]
            ta46=df['الأبواب'].astype(str).str.split('-',expand=True)
            cl=ta46[1].str.replace('\d+','')
            ma46['البيان']=cl.str.strip()
            ma46['الكود (ISIC 4)']=ma46['الكود (ISIC 4)'].replace(['38','43','73','82'],'32')
            ma46['البيان']=ma46['البيان'].replace(['أنشطة جمع النفايات ومعالجتها وتصريفها ، واسترجاع المواد','أنشطة التشييد المتخصصة','أبحاث الإعلان والسوق','الأنشطة الإدارية للمكاتب ، وأنشطة الدعم للمكاتب وغير ذلك من أنشطة الدعم للأعمال'],'الصناعة التحويلية الأخرى')
            ma46['كويتي']=df['الإدارة العليا والإدارة المساعدة-أجنبي']
            ma46['خليجي']=df['العمالة التي يتم تشغيلها في الأعمال الإضافية-خليجي']
            ma46['عربي']=df['العمالة التي يتم تشغيلها في الأعمال الإضافية-عربي']
            ma46['أجنبي']=df['العمالة التي يتم تشغيلها في الأعمال الإضافية-أجنبي']
            col46=['كويتي','خليجي','عربي','أجنبي']
            ma46[col46]=ma46[col46].apply(pd.to_numeric, errors='coerce').fillna(0).astype(np.int64,errors='ignore')
            ma46['الإجمالي']=ma46['كويتي']+ma46['خليجي']+ma46['عربي']+ma46['أجنبي']
            tab46=pd.DataFrame({})
            if option == 'Sum':
                tab46=ma46.groupby(['الكود (ISIC 4)','البيان'])[['كويتي','خليجي','عربي','أجنبي','الإجمالي']].sum()
            if option == 'Mean':
                tab46=ma46.groupby(['الكود (ISIC 4)','البيان'])[['كويتي','خليجي','عربي','أجنبي','الإجمالي']].mean()
            tab46.reset_index(inplace=True)
            tab46.loc['إجمالي المنشآت الصناعية']=tab46.sum(numeric_only=True, axis=0)
            tab46['الكود (ISIC 4)']=tab46['الكود (ISIC 4)'].replace(np.nan,'')
            tab46['البيان']=tab46['البيان'].replace(np.nan,'إجمالي المنشآت الصناعية')
            tab46=tab46.reset_index(drop=True)
            col46=['كويتي','خليجي','عربي','أجنبي','الإجمالي']
            tab46[col46]=tab46[col46].astype(np.int64,errors='ignore')
            da46=tab46.style.apply(highlight_col, axis=None)
            
            # table47
            ta47=df['الأبواب'].astype(str).str.split('-',expand=True)
            ma47=pd.DataFrame({})
            ma47['الكود (ISIC 4)'] = df['ISIC4'].astype(str).str[:2]
            ta47=df['الأبواب'].astype(str).str.split('-',expand=True)
            cl=ta47[1].str.replace('\d+','')
            ma47['البيان']=cl.str.strip()
            ma47['الكود (ISIC 4)']=ma47['الكود (ISIC 4)'].replace(['38','43','73','82'],'32')
            ma47['البيان']=ma47['البيان'].replace(['أنشطة جمع النفايات ومعالجتها وتصريفها ، واسترجاع المواد','أنشطة التشييد المتخصصة','أبحاث الإعلان والسوق','الأنشطة الإدارية للمكاتب ، وأنشطة الدعم للمكاتب وغير ذلك من أنشطة الدعم للأعمال'],'الصناعة التحويلية الأخرى')
            ma47['كويتي']=df['إجمالي-كويتي']
            ma47['خليجي']=df['إجمالي-خليجي']
            ma47['عربي']=df['إجمالي-عربي']
            ma47['أجنبي']=df['إجمالي-أجنبي']
            col47=['كويتي','خليجي','عربي','أجنبي']
            ma47[col47]=ma47[col47].apply(pd.to_numeric, errors='coerce').fillna(0).astype(np.int64,errors='ignore')
            ma47['الإجمالي']=ma47['كويتي']+ma47['خليجي']+ma47['عربي']+ma47['أجنبي']
            tab47=pd.DataFrame({})
            if option == 'Sum':
                tab47=ma47.groupby(['الكود (ISIC 4)','البيان'])[['كويتي','خليجي','عربي','أجنبي','الإجمالي']].sum()
            if option == 'Mean':
                tab47=ma47.groupby(['الكود (ISIC 4)','البيان'])[['كويتي','خليجي','عربي','أجنبي','الإجمالي']].mean()
            tab47.reset_index(inplace=True)
            tab47.loc['إجمالي المنشآت الصناعية']=tab47.sum(numeric_only=True, axis=0)
            tab47['الكود (ISIC 4)']=tab47['الكود (ISIC 4)'].replace(np.nan,'')
            tab47['البيان']=tab47['البيان'].replace(np.nan,'إجمالي المنشآت الصناعية')
            tab47=tab47.reset_index(drop=True)
            col47=['كويتي','خليجي','عربي','أجنبي','الإجمالي']
            tab47[col47]=tab47[col47].astype(np.int64,errors='ignore')
            da47=tab47.style.apply(highlight_col, axis=None)
 ...
 'الإجمالي'], title="Bar chart for قيمة الإنفاق للمنشآت الصناعية في عام 2020 (ألف دينار)",width=800, height=700)
            ...
            #table43      
            st.subheader('Table 43')
            st.subheader("العمالة لدى لقطاع الصناعي (الإدارة العليا والإدارة المساعدة)")
            st.table(tab43.style.apply(highlight_col, axis=None).set_precision(0))
            downloaded_file = da43.to_excel(towrite, encoding='utf-8', index=False, header=True ) # write to BytesIO buffer
            towrite.seek(0)  # reset ponp.int64er
            b64 = base64.b64encode(towrite.read()).decode() 
            linko= f'<a href="data:application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;base64,{b64}" download="Table_43.xlsx">Download excel file</a>'
            st.markdown(linko, unsafe_allow_html=True)
            
            st.markdown("**Figure 50 : Bar chart for العمالة لدى لقطاع الصناعي (الإدارة العليا والإدارة المساعدة) based on 'Code (ISIC 4)' every year**".format(option))
            
            fig50 = px.bar(tab43, x="البيان", y="الكود (ISIC 4)",color="البيان",hover_data=['كويتي', 'خليجي', 'عربي', 'أجنبي', 'الإجمالي'], title="Bar chart for العمالة لدى لقطاع الصناعي (الإدارة العليا والإدارة المساعدة)",width=800, height=700)

            st.write(fig50)
            
            #table44      
            st.subheader('Table 44')
            st.subheader("العمالة لدى لقطاع الصناعي (الإدارة الفنية وعمالة الإنتاج)")
            st.table(tab44.style.apply(highlight_col, axis=None).set_precision(0))
            downloaded_file = da44.to_excel(towrite, encoding='utf-8', index=False, header=True ) # write to BytesIO buffer
            towrite.seek(0)  # reset ponp.int64er
            b64 = base64.b64encode(towrite.read()).decode() 
            linko= f'<a href="data:application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;base64,{b64}" download="Table_44.xlsx">Download excel file</a>'
            st.markdown(linko, unsafe_allow_html=True)
            
            st.markdown("**Figure 51 : Bar chart for العمالة لدى لقطاع الصناعي (الإدارة الفنية وعمالة الإنتاج) based on 'Code (ISIC 4)' every year**".format(option))
            
            fig51 = px.bar(tab44, x="البيان", y="الكود (ISIC 4)",color="البيان",hover_data=['كويتي', 'خليجي', 'عربي', 'أجنبي', 'الإجمالي'], title="Bar chart for العمالة لدى لقطاع الصناعي (الإدارة الفنية وعمالة الإنتاج)",width=800, height=700)

            st.write(fig51)
            
            #table45      
            st.subheader('Table 45')
            st.subheader("العمالة لدى لقطاع الصناعي (عمال الخدمات والصيانة)")
            st.table(tab45.style.apply(highlight_col, axis=None).set_precision(0))
            downloaded_file = da45.to_excel(towrite, encoding='utf-8', index=False, header=True ) # write to BytesIO buffer
            towrite.seek(0)  # reset ponp.int64er
            b64 = base64.b64encode(towrite.read()).decode() 
            linko= f'<a href="data:application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;base64,{b64}" download="Table_45.xlsx">Download excel file</a>'
            st.markdown(linko, unsafe_allow_html=True)
            
            st.markdown("**Figure 52 : Bar chart for العمالة لدى لقطاع الصناعي (عمال الخدمات والصيانة) based on 'Code (ISIC 4)' every year**".format(option))
            
            fig52 = px.bar(tab45, x="البيان", y="الكود (ISIC 4)",color="البيان",hover_data=['كويتي', 'خليجي', 'عربي', 'أجنبي', 'الإجمالي'], title="Bar chart for العمالة لدى لقطاع الصناعي (عمال الخدمات والصيانة)",width=800, height=700)

            st.write(fig52)
            
            #table46      
            st.subheader('Table 46')
            st.subheader("العمالة لدى لقطاع الصناعي (العمالة التي يتم تشغيلها في الأعمال الإضافية)")
            st.table(tab46.style.apply(highlight_col, axis=None).set_precision(0))
            downloaded_file = da46.to_excel(towrite, encoding='utf-8', index=False, header=True ) # write to BytesIO buffer
            towrite.seek(0)  # reset ponp.int64er
            b64 = base64.b64encode(towrite.read()).decode() 
            linko= f'<a href="data:application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;base64,{b64}" download="Table_46.xlsx">Download excel file</a>'
            st.markdown(linko, unsafe_allow_html=True)
            
            st.markdown("**Figure 53 : Bar chart for العمالة لدى لقطاع الصناعي (العمالة التي يتم تشغيلها في الأعمال الإضافية) based on 'Code (ISIC 4)' every year**".format(option))
            
            fig53 = px.bar(tab46, x="البيان", y="الكود (ISIC 4)",color="البيان",hover_data=['كويتي', 'خليجي', 'عربي', 'أجنبي', 'الإجمالي'], title="Bar chart for العمالة لدى لقطاع الصناعي (العمالة التي يتم تشغيلها في الأعمال الإضافية)",width=800, height=700)

            st.write(fig53)
            
            #table47      
            st.subheader('Table 47')
            st.subheader("إجمالي العمالة لدى لقطاع الصناعي")
            st.table(tab47.style.apply(highlight_col, axis=None).set_precision(0))
            downloaded_file = da47.to_excel(towrite, encoding='utf-8', index=False, header=True ) # write to BytesIO buffer
            towrite.seek(0)  # reset ponp.int64er
            b64 = base64.b64encode(towrite.read()).decode() 
            linko= f'<a href="data:application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;base64,{b64}" download="Table_47.xlsx">Download excel file</a>'
            st.markdown(linko, unsafe_allow_html=True)
            
            st.markdown("**Figure 54 : Bar chart for إجمالي العمالة لدى لقطاع الصناعي based on 'Code (ISIC 4)' every year**".format(option))
            
            fig54 = px.bar(tab47, x="البيان", y="الكود (ISIC 4)",color="البيان",hover_data=['كويتي', 'خليجي', 'عربي', 'أجنبي', 'الإجمالي'], title="Bar chart for إجمالي العمالة لدى لقطاع الصناعي",width=800, height=700)
...
#master excel             
            ...
            with pd.ExcelWriter(towrite, engine="xlsxwriter") as writer:
                ...
                da43.to_excel(writer, sheet_name="43", startrow = 2,index=False)
                da44.to_excel(writer, sheet_name="44", startrow = 2,index=False)
                da45.to_excel(writer, sheet_name="45", startrow = 2,index=False)
                da46.to_excel(writer, sheet_name="46", startrow = 2,index=False)
                da47.to_excel(writer, sheet_name="47", startrow = 2,index=False)
                ...
                worksheet43 = writer.sheets['43']
                worksheet44 = writer.sheets['44']
                worksheet45 = writer.sheets['45']
                worksheet46 = writer.sheets['46']
                worksheet47 = writer.sheets['47']
                ...
                worksheet43.write(0, 5, 'العمالة لدى لقطاع الصناعي (الإدارة العليا والإدارة المساعدة)', workbook.add_format({'bold': True, 'color': '#0d0d0c', 'size': 11}))
                worksheet44.write(0, 5, 'العمالة لدى لقطاع الصناعي (الإدارة الفنية وعمالة الإنتاج)', workbook.add_format({'bold': True, 'color': '#0d0d0c', 'size': 11}))
                worksheet45.write(0, 5, 'العمالة لدى لقطاع الصناعي (عمال الخدمات والصيانة)', workbook.add_format({'bold': True, 'color': '#0d0d0c', 'size': 11}))
                worksheet46.write(0, 6, 'العمالة لدى لقطاع الصناعي (العمالة التي يتم تشغيلها في الأعمال الإضافية)', workbook.add_format({'bold': True, 'color': '#0d0d0c', 'size': 11}))
                worksheet47.write(0, 5, 'إجمالي العمالة لدى لقطاع الصناعي', workbook.add_format({'bold': True, 'color': '#0d0d0c', 'size': 11}))
